...
depScaled=scale2.fit_transform(deptime.reshape(y.size,1))
...
# X=np.hstack((x0.reshape(y.size,1), depScaled, timediffScaled))
...
# # plt.show()
# threshold=0.5
# weights2=np.array([-0.9145271, 0.1713717, 2.42742371]).reshape(3,1)
# accu=logistic_accuracy(df_test, y_test, weights, threshold)
# y_pred=logistic_predict(df_test, weights, threshold)
...
# faulty=sigmoid(df_test[idx,:].dot(weights2))
# print(faulty[0:10])
# faulty2=sigmoid(df_test[idx2, :].dot(weights2))
# print(faulty2[0:10])
...
cost_history, weights=svm_train(df_train, y_train, initialWeights, reg=0.075, learning_rate=5e-2, iterations=2000)
plt.plot(cost_history)
plt.xlabel("epochs")
plt.ylabel("Cost")
plt.show()
